...
const perfMarkers = {
  serialPort: {
    open_begin: 'open-serial-port-begin',
    open_end: 'open-serial-port-end',
    open_label: 'open-serial-port-begin to open-serial-port-end'
  },
  deviceConnect: {
  ...
  deviceRequest: {
    open_begin: 'open-device-request-begin',
    open_end: 'open-device-request-end',
    open_label: 'open-device-request-begin to open-device-request-end'
  },
  configurations: {
...
HvcP2.prototype._openSerialPort = function(p) {
	let promise = new Promise((resolve, reject) => {
		performance.mark(perfMarkers.serialPort.open_begin);
