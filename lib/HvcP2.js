				...
				if(res['error']) {
					...
					if (cn ===4 ) {
						...
						performance.mark(perfMarkers.deviceRequest.end);
          				performance.measure(perfMarkers.deviceRequest.label, perfMarkers.deviceRequest.begin, perfMarkers.deviceRequest.end);			performance.mark(perfMarkers.deviceRequest.begin);
					}
					resolve(res['data']);
				}
			}
...
HvcP2.prototype.detect = function(params) {
	let promise = new Promise((resolve, reject) => {
		performance.mark(perfMarkers.detect.begin);
		this.request(0x04, params).then((res) => {
			if(params['imageType']) {
				...
				mHvcP2Image.conv(opts, res).then((result) => {
					...
					delete res['image']['buffer'];
					if(params['imageType'] === 1) {
						res['image']['buffer'] = result;
					} else if(params['imageType'] === 2) {
						res['image']['dataUrl'] = result;
					}
					performance.mark(perfMarkers.detect.end);
					performance.measure(perfMarkers.detect.label, perfMarkers.detect.begin, perfMarkers.detect.end);
					resolve(res);
				}).catch((error) => {
					console.log("427 buffering data error:");
					console.log(error);
					reject(error);
				});
			} else {
