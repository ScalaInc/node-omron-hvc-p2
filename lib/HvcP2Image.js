...
let Jimp = null;
try {
	// Fuck lwip - use JIMP! BIG JIMP!
	Jimp = require("jimp");
} catch(e) {}

console.log("Image Library in use: node-gd: " + !!mGd + " - lwip: " + !!mLwip + " - jimp: "+ !!Jimp);
...
HvcP2Image.prototype.conv = function(params, result) {
	let promise = new Promise((resolve, reject) => {
		if(!mGd && !mLwip && !Jimp) {
			reject(new Error('At least one of the node modules `node-gd` or (Linux/MacOS), `jimp`(Linux/MacOS/Windows), or `lwip` (Windows) must be installed.'));
			return;
		}
					...
					result['face'].forEach((o) => {
						if('face' in o) {
							...
							if('age' in o) {
								...
								//'/Users/rhall/Documents/git_repos/github/svcs-target-poc-omron/FreeMono.ttf';
...
// Fuck LWIP - dont need these fuckers anymore
