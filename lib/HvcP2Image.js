...
HvcP2Image.prototype._convArrayToImageSharp = function(p, result) {
	...
	let promise = new Promise((resolve, reject) => {
		...
		// buffer the data
		image = sharp(p['buffer'], { "density":72, raw: { width: p['width'], height: p['height'], channels: 1} })
		.png()
		.toBuffer(function(err, data) { 	
			...
			if(p['marker'] === true) {
				...
				let svgText = '<svg width="320" height="240">';
			...
			if(p['marker'] === true) {
				...
				svgBuffer = Buffer(svgText);
				// cutout: true					
				let svgImg = sharp(data)
