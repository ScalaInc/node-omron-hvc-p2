...
// What are we using?
console.log("Image Library in use: node-gd: " + !!mGd + " | lwip: " + !!mLwip + " | jimp: "+ !!Jimp+ " | sharp: "+ !!sharp);
...
HvcP2Image.prototype._convArrayToImageJimp = function(p, result) {
	...
	let promise = new Promise((resolve, reject) => {
		...
		var image = new Jimp(p['width'], p['height'], function (error, image) {
		//console.log(error);
			let left = 0;
			let top = 0;
			let index = 0;
...
HvcP2Image.prototype._convArrayToImageSharp = function(p, result) {
	...
	let promise = new Promise((resolve, reject) => {
		...
		var image = new sharp(p['buffer'], { "density":72, raw: { width: p['width'], height: p['height'], channels: 1} }).png().toFile(fpath+p['format'], function(err, info) {
				//console.log(info);
				if(err) {
					console.log(err);
					reject(err);
				...
				if('face' in result) {
					result['face'].forEach((o) => {
						if('face' in o) {
							...
							var svgBuffer = Buffer.from("<svg width='"+(x1+1)+"' height='"+x1+"'><g><rect fill='rgba(0,0,0,0)' stroke-width='1' stroke='"+"#00ff00"+"' height='"+x1+"' width='"+(y1-1)+"'></rect> </g> </svg>");
							
							var svgImg = sharp( svgBuffer, {"density":25}).png() 					
							.toFile(fpath+"_overlay."+p['format'], function(err, info) { //.toFile(fpath+p['format']
									//console.log(info);
									if(err) {
										console.log(err);
										reject(err);
							...
							//sharp(image, { raw: { width: p['width'], height: p['height'], channels: 3} })
							//sharp(image).overlayWith(svgImg, {top: x2 , left: y2 });
							
							/*
							.png().toFile(fpath+p['format'], function(err, info) { //.toFile(fpath+p['format']
									//console.log(info);
									if(err) {
										console.log(err);
										reject(err);
