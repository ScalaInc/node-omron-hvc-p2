...
HvcP2Image.prototype._convArrayToImageGd = function(p, result) {
	...
	let promise = new Promise((resolve, reject) => {
		mGd.createTrueColor(p['width'], p['height'], (error, image) => {
					...
					result['face'].forEach((o) => {
						if('face' in o) {
							...
							if('age' in o) {
								...
								let font = s_path.resolve(mypath, "fonts/FreeMono.ttf");
			...
			let fpath = s_path.resolve(mypath, "data/image.");
	...
	let promise = new Promise((resolve, reject) => {
		...
		.toBuffer(function bufferingImage(err, data) { 	
			...
			if(p['marker'] === true) {
				...
				// revise this to allow for vertical orientation when camera angle has been rotated - doh!
				// fixed to derive from above - but assuming we are getting 320x240 - need to accomodate 160x120 option...
				svgText =  '<svg width="'+(w/5)+'" height="'+(h/5)+'">';
				svgText += '<rect fill="rgba(0,0,0,0)"  x="0" y="0" width="'+(w/5)+'" height="'+(h/5)+'"/>';
			...
			if(p['marker'] === true) {
				...
				sharp(data)
				.on('error', err => console.log(`${err.message} from sharp`))
				.overlayWith( svgBuffer, {"density":72, width: (p['width']/5), height: (p['height']/5)} ) // was 320x240 - need to fix those fixed sizes too!
				.on('error', err => console.log(`${err.message} from sharp`))
				.png({"compressionLevel":9}).on('error', err => console.log(`${err.message} from sharp`))
				//	.resize(320,240)
